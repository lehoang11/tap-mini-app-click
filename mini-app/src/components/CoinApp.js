...
export default function CoinApp(props) {
  ...
  useEffect(() => {
    const req = async () => {
      await axios.post(`${BASE_API_URL}user/${userId}/add-point`, {
  ...
  useEffect(() => {
    const req = async () => {
      await axios.get(`${BASE_API_URL}user/${userId}`)
      .then(response => {
      ...
      await axios.get(`${BASE_API_URL}user/${userId}/get-rank`)
      .then(response => {
      ...
      await axios.get(`${BASE_API_URL}leaderboard`)
      .then(response => {
  ...
  const handleWithdraw = () => {
    if (pointCount >= 50) {
      axios.post(`${BASE_API_URL}withdraw`, {
        userId: userId,
